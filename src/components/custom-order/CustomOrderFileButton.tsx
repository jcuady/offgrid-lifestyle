...
export function CustomOrderFileButton({ fileKey, fileName, label, className }: CustomOrderFileButtonProps) {
  if (!fileName) {
    return <span className="text-sm text-offgrid-green/50">No file uploaded</span>;
  }

  if (!hasCustomOrderFile(fileKey)) {
    return (
  ...
  return (
    <button
      type="button"
      onClick={() => void downloadCustomOrderFile(fileKey!, fileName)}
      className={cn(
